...
- **The bridge PDA no longer needs a SOL float.** Every outbound path calls `HelperProgram.swap_gas_to_lamports(rent)` right before its CPI (`_fundMessageRent`): in the rome-evm program that is a System transfer from the operator's payer to the bridge's PDA, executed with `refund_to_signer`, so the operator fronts exactly the message-account rent and the user is billed exactly that in gas, atomically. Rents are owner-settable (`setMessageRents`; defaults measured on Hadrian 2026-09-21: CCTP 2,824,480, Wormhole 2,477,770 + 10 fee). Circle's rent stays reclaimable (`reclaimEventAccount`, signed by `event_rent_payer`) — nothing calls it yet; Wormhole's is not.
