...
### Cold-ledger mainnet deploy (rubicon, chain 7531)

Mainnet deploys move off the hot-key to a cold Ledger via the **hot-deploy + admin-handover** flow (validated on trajan 121302, 2026-06-16). Phase-6 has only two transferable admin roles — `OracleAdapterFactory.owner` and `UniswapV2Factory.feeToSetter`; every other contract is permissionless or immutable. So the recommended flow is:

1. Hot-deploy the whole stack with a throwaway key (existing `scripts/*` flow). Tap-free.
2. Verify deployed bytecode against audited contracts.
3. Hand the two roles to the cold Ledger via `scripts/transfer-admin-to-ledger.ts` — signed by the current hot owner, single-step → also tap-free. The Ledger only signs when it *later* exercises a role.
4. Discard the hot key.
...
The Ledger-as-deployer path (`DEPLOY_VIA_LEDGER=1 ... npx tsx scripts/deploy-ledger.ts`) is also implemented and devnet-validated (`ERC20SPLFactory` at `0x7659…` on trajan), but it requires ~20-40 device taps for the full Phase-6 stack — prefer hot-deploy + handover. Scripts:

- `scripts/lib/ledger.ts` — viem-native Ledger adapter (no `hardhat-ledger` dep; `@ledgerhq/hw-app-eth` + `hw-transport-node-hid`).
- `scripts/lib/deployer.ts` — `getDeployer()` returns the Ledger wallet client when `DEPLOY_VIA_LEDGER=1`, else the hot-key wallet client.
- `scripts/deploy-ledger.ts` — Ledger-signed deploy entrypoint (must run under `tsx`, not `hardhat run` — HH3 CJS hits an ESM require-cycle in `@ledgerhq`).
- `scripts/transfer-admin-to-ledger.ts` — single-step handover.
- `scripts/test-handover.ts` / `scripts/test-ledger-adapter.ts` — devnet validation harness.

Full runbook (rationale, devnet rehearsal steps, fallback via `cast send --ledger`, Blind-Signing pre-flight): [`COLD_LEDGER_MAINNET_DEPLOY.md`](COLD_LEDGER_MAINNET_DEPLOY.md).
...
- `hadrian` (200010, testnet) — full stack: `ERC20SPLFactory`, OG-V2 (Pyth + Switchboard feeds, BatchReader), `RomeBridgeWithdraw`, `SPL_ERC20_*` wrappers
...
- `hadrian` (200010), `trajan` (121302), `nerva` (210000) — current Rome network entries in `hardhat.config.ts`.
...
| `RomeBridgeWithdraw.bridgeOutToSolana` / `ensureRecipientAta` (mint-agnostic 3-arg / 2-arg signatures since #227) and `SPL_ERC20.balanceOf` | **rome-ui** `src/features/bridge/hooks/useOutboundSplBridge.ts`, `useBalances.ts`, `useRomeHoldings.ts`. ABI is parseAbi-encoded inline; no JSON to regenerate. The legacy `SPL_ERC20.bridgeOutToSolana(bytes32, uint256)` 1-arg signature still exists on the wrapper but is not the active path. |
...
| `SPL_ERC20` | `bridgeOutToSolana(bytes32 recipient, uint256 value) → bool` + `ensureRecipientAta(bytes32 recipient)` | Legacy gas-mint-only path kept on the wrapper for back-compat. rome-ui now uses the `RomeBridgeWithdraw` mint-agnostic variants. |
...
| `SimpleActivator` | `topUpUserPda(uint64 lamports) external payable` (refill the user's PDA SOL reserve when below the fresh-recipient transfer threshold) | rome-ui top-up flow (e.g. before first outbound CCTP burn) |
