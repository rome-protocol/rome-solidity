...
npx hardhat run scripts/deploy_meteora_router.ts --network <chain>
npx hardhat run scripts/register_meteora_pool.ts --network <chain>
...
| Helper Program | `0xff..09` | `IHelperProgram` — SPL / PDA / ATA / lamports / gas-token plumbing (~25 selectors) | legacy | `program/src/non_evm/helper.rs` |
| Ed25519 | `0xff..0a` | `IEd25519.verify_from_allowlist(bytes32[],bytes,uint8,uint8) view` — verify the Solana runtime checked an Ed25519 sig in the current tx; returns the verified pubkey from the allowlist | neutral (read-only) | `program/src/non_evm/ed25519.rs` |
...
- CrossStateEthCall reads (`account_data_at`, `account_info`, `user_balance`, `allowance_of`, `ISplCached.account`, etc.) are track-neutral — pure reads, never lock a track
...
| `0x46efa679` | `transfer_spl_to_signer(uint64 amount, bytes32 mint)` | **Return-to-signer leg** (PR #223, 2026-05-29). Source = `ata(external_auth(caller), mint)`; destination = `ata(signer, mint)` where `signer` is the outer Solana tx signer (the user's own Solana wallet, not a PDA). Signs as `external_auth(caller)`. Return leg for Solana-native users (`do_tx_unsigned` / `activate_ata` flow); any mint. |
...
- **`contracts/oracle/`** — Oracle Gateway V2: Chainlink-compatible adapters for Pyth Pull and Switchboard V3 price feeds, plus EVM-side cache adapters. `OracleAdapterFactory` deploys `PythPullAdapter`, `SwitchboardV3Adapter`, `CachedPythAdapter`, and the generic `CachedFeedAdapter` instances via EIP-1167 minimal proxy clones (7-arg constructor since #224). Each direct adapter reads Solana account data via CPI precompile, parses Borsh-encoded price data (`PythPullParser` / `SwitchboardParser`), and normalizes to 8-decimal Chainlink format. **Cached adapters (#224, 2026-05-30):** `CachedPythAdapter` is a keeper-refreshed EVM-side cache over a Pyth Pull Solana account — `refresh()` parses + SSTOREs the 8-decimal price; `latestRoundData()` is a cheap SLOAD. `CachedFeedAdapter` is a source-agnostic cache decorator over ANY `AggregatorV3Interface` underlying (PythPull / Switchboard / Chainlink). Measured on Hadrian: ~137K CU cached read vs ~509K direct Pyth-Pull read — moves the Borsh parse to the keeper's tx, off the consumer's path. Cold-start reverts with `UninitializedPriceFeed`; aged-out reverts with `StalePriceFeed`. Refresh script: `scripts/oracle/refresh-cached-feeds.ts` (#225). `IExtendedOracleAdapter` extends `IAggregatorV3Interface` with confidence intervals, EMA data, and price status. `BatchReader` reads multiple feeds in one call. The factory includes owner-controlled pause/unpause emergency controls. Includes `examples/SampleLendingOracle.sol`. **Pyth Lazer was removed (#196)** — no Lazer adapter / cache / `lazer_price` selector exists in the current tree.
- **`contracts/bridge/`** — Rome Bridge: outbound (Rome → Sepolia + Rome → Solana). Sole contract is **`RomeBridgeWithdraw`** — emits Wormhole Token Bridge or CCTP outbound messages via CPI signed as the user's PDA. Surface: `burnUSDC(uint256, address ethereumRecipient)` (CCTP outbound), `approveBurnETH(uint256)` + `burnETH(uint256, address)` (Wormhole outbound, two-tx by 1.4M-CU constraint), and **mint-agnostic Rome → Solana SPL egress** added in PR #227 (2026-06-01): `bridgeOutToSolana(bytes32 recipient, uint256 amount, bytes32 mint)` + `ensureRecipientAta(bytes32 recipient, bytes32 mint)`. The 3-arg egress is transfer-only via `HelperProgram.transfer_spl` (legacy track, signed as `external_auth(caller)`); two single-CPI txs avoid `CpiProhibitedInIterativeTx`. Inherits `ERC2771Context` but deploys with `forwarder = address(0)` (user-paid since #227 dropped the paymaster). `IWormholeTokenBridge.sol` and `ICCTP.sol` encode the native/Anchor Solana instructions. All Solana pubkeys supplied via constructor params so the contract is network-agnostic. `RomeBridgeEvents.sol` carries shared `BridgedOutToSolana` event etc. **Removed in #227:** `RomeBridgePaymaster.sol` (legacy EIP-2771 sponsored bridge — unused, rome-ui signs directly user-paid) and `RomeBridgeInbound.sol` (legacy inbound — superseded by `settle_inbound_bridge` on rome-evm-private). **See `contracts/bridge/README.md`** for architecture + the problems-and-fixes runbook; **`SOLANA_EGRESS_DESIGN.md`** for the egress design.
...
| Bridge contracts | `contracts/bridge/RomeBridgeWithdraw.sol` + `contracts/bridge/README.md` + `SOLANA_EGRESS_DESIGN.md` (the only bridge contract — Paymaster + Inbound removed in #227) |
| Activator | `contracts/activation/SimpleActivator.sol` (single-tx `activate{value: activationCost}()` user-paid bootstrap) |
...
Deployment metadata is tracked in `deployments/{network}.json`, written by the Hardhat scripts on each `npx hardhat run`. The local stack file `local.json` is generated by `scripts/setup-local.ts` and should not be committed (regenerated per local stack restart). Per-chain devnet/testnet receipts are committed alongside their hardhat network entry.

**Current deployments** (`deployments/`): `trajan.json` (devnet), `hadrian.json` (testnet), `nerva.json` (testnet — Solana testnet target, added #229). The CHANGELOG.md preserves history for retired chains. New chains add themselves back via `/bring-up-chain` Row 6 (`/deploy-solidity`).

### Networks

Defined in `hardhat.config.ts`:

- `trajan` — chainId `121302`, Rome devnet — `https://trajan.devnet.romeprotocol.xyz/` (key: `TRAJAN_PRIVATE_KEY`)
- `nerva` — chainId `210000`, Rome testnet (Solana testnet target) — `https://nerva.testnet.romeprotocol.xyz/` (key: `NERVA_PRIVATE_KEY`, #229)
- `hadrian` — chainId `200010`, Rome testnet (Solana devnet target) — `https://hadrian.testnet.romeprotocol.xyz/` (key: `HADRIAN_PRIVATE_KEY`)
...
**Decommissioned (removed from hardhat config):** `aurelius` (30001, real-testnet, #228 — 2026-06-04), `augustus` (200001) + `marcus` (121301) (#229 — 2026-06-04), `maximus` (121215, #90), `subura` (121222), `esquiline` (121225), `cassius` (121228), `cassius-test` (121298), `monti_spl` (legacy subura-proxy alias). See CHANGELOG.md for per-chain deploy history.
...
| `RomeBridgeWithdraw.bridgeOutToSolana(bytes32,uint256,bytes32)` / `ensureRecipientAta(bytes32,bytes32)` (mint-agnostic, #227) | **rome-ui** `src/features/bridge/hooks/useOutboundSplBridge.ts`. ABI is parseAbi-encoded inline; no JSON to regenerate. |
| `SPL_ERC20.bridgeOutToSolana(bytes32,uint256)` / `ensureRecipientAta(bytes32)` / `balanceOf` (mint-fixed per-wrapper surface) | **rome-ui** `useBalances.ts`, `useRomeHoldings.ts` (for `balanceOf`). The egress 2-arg variants are legacy-shape — primary outbound path is the 3-arg variant on `RomeBridgeWithdraw`. |
...
| `RomeBridgePaymaster` / `RomeBridgeInbound` | **REMOVED in #227** — files deleted. Inbound flow is `settle_inbound_bridge` on rome-evm-private. rome-ui may still carry these keys in `chain.contracts` config for back-compat parsing of older chains.yaml — no active call sites; drop on next rome-ui cleanup. |
| Oracle adapter interfaces | Consuming contracts in this repo that use the adapters |
| `CachedPythAdapter` / `CachedFeedAdapter` / `OracleAdapterFactory.createCachedPythFeed` / `createCachedFeed` (#224) | Keeper service `scripts/oracle/refresh-cached-feeds.ts` (#225) must refresh on the cadence consumers tolerate (`maxStaleness` config); rome-ops deploys the keeper. Compound-on-Rome's collateralization reads benefit most from the SLOAD path. |
...
|---|---|---|
| `RomeBridgeWithdraw` | `bridgeOutToSolana(bytes32 recipient, uint256 amount, bytes32 mint) → BridgedOutToSolana event` (mint-agnostic egress, PR #227, 2026-06-01) | `src/features/bridge/hooks/useOutboundSplBridge.ts` (Rome → Solana outbound for any wrapper) |
| `RomeBridgeWithdraw` | `ensureRecipientAta(bytes32 recipient, bytes32 mint)` | same hook (preflight — single CPI ATA-create paid by sender's unified user PDA) |
| `SPL_ERC20` | `bridgeOutToSolana(bytes32 recipient, uint256 value) → bool` + `ensureRecipientAta(bytes32 recipient)` (mint-fixed; legacy per-wrapper surface; still present) | not the primary consumer path post-#227 — rome-ui calls the mint-agnostic `RomeBridgeWithdraw` variant |
| `SPL_ERC20` | `balanceOf(address) → uint256` (reads AUTHORITY_PDA's ATA, not `_accounts` map) | wagmi multicall, `useChainTokenBalances`, every Portfolio row |
...
| `SimpleActivator` | `activate() external payable` (single-tx: PDA fund + wUSDC + wSOL ATA creates + ERC20Users registrations) | `src/features/portfolio/components/ActivateAccountButton.tsx` (first-time PDA activation; primary CTA replacement on Swap/Bridge/Liquidity until activated) |
...
| `SimpleActivator` | `topUpUserPda(uint64 lamports) external payable` | post-activation top-ups for ongoing rent on transient bridge accounts |
...
`RomeBridgePaymaster` and `RomeBridgeInbound` were **DELETED in PR #227** (2026-06-01) — they were legacy paths (the paymaster's EIP-2771 sponsorship was unused; inbound moved to `settle_inbound_bridge` on rome-evm-private signed by `SOLANA_SETTLE_PAYER_KEY`). If rome-ui's `chain.contracts` config still carries `romeBridgePaymaster` / `romeBridgeInbound` keys, those are back-compat-only parsing — no live call sites. The current inbound flow is `settle_inbound_bridge` on rome-evm-private after Circle/Wormhole `receiveMessage` confirms — no Rome EVM tx involved.
...
- `SimpleActivator.activate{value: activationCost}()` is a **single-tx** user-paid bootstrap (post-consolidation per #162 + 0acabea — collapsed from the prior three-tx shape via HelperProgram CPI shortcuts). Funds the user's unified PDA at the rent-exempt floor (890,880 lamports), creates the user's wUSDC + wSOL ATAs, and registers in both wrappers' `ERC20Users` mappings — all atomic, fits within the 1.4M CU envelope. Strict equality on `msg.value` against `activationCost()`. Idempotent (no-ops on re-call). Sybil resistance: user pays themselves; no operator subsidy. The previously-distinct `PAYER_PDA` (salted at `[EXTERNAL_AUTHORITY, evmAddr, "PAYER"]`) collapsed into the unified PDA at `find_program_address([EXTERNAL_AUTHORITY, evmAddr])` — that PDA signs CPIs, owns ATAs, and holds rent funds for transient bridge accounts. `topUpUserPda(uint64 lamports) payable` exists for ongoing top-ups after activation.
