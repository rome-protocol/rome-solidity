...
**`ERC20SPLFactory` mint creation is atomic (#326, 2026-08).** `create_token_mint()` creates AND initializes the SPL mint in a single `HelperProgram.create_and_init_mint` dispatch — there is never a created-but-uninitialized window, and the mint authority is bound to the caller. Prior split flow (`create_token_mint` → separate `init_token_mint(bytes32)`) exposed a mint-authority front-run: a third party could `init_token_mint` someone else's freshly-created (uninitialized) mint and seize issuance. `init_token_mint(bytes32)` is retained as an **inert `view` shim** for backward compatibility — it requires `is_initialized == 1` and performs no state change, so old two-step callers succeed as a no-op against an already-initialized mint. If you're writing new code, call `create_token_mint()` and stop; do not add a follow-up `init_token_mint` call. Regression test: `tests/erc20spl_factory_hijack.poc.ts` (Hadrian live acceptance). Rubicon (mainnet) redeployed at the fix in #327 — the previous factory (`0x888a…5409`) is superseded; consumers point at the new factory via the registry.
...
| `ERC20SPLFactory` (mint creation / `create_token_mint` / `init_token_mint`) | `tests/erc20spl_factory.integration.ts` + `tests/erc20spl_factory_hijack.poc.ts` (Hadrian live acceptance — asserts the #326 atomic-init fix and the inert-view backward-compat shim) |
