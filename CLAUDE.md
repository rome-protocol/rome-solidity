...
# Run tests (requires local Rome-EVM node or marcus devnet)
...
npx hardhat run scripts/oracle/deploy-factory.ts --network marcus
npx hardhat run scripts/oracle/deploy-and-test.ts --network marcus  # end-to-end deploy + test

# Test oracle feeds on live network
npx hardhat run scripts/oracle/test-feeds.ts --network marcus       # Pyth v1 feeds
npx hardhat run scripts/oracle/test-feeds-v2.ts --network marcus    # Oracle Gateway V2 (Pyth Pull + batch reader)
npx hardhat run scripts/oracle/test-switchboard.ts --network marcus # Switchboard V2 feeds
...
npx hardhat run scripts/oracle/check-account-owner.ts --network marcus
npx hardhat run scripts/oracle/check-switchboard.ts --network marcus
...
Deployment metadata is tracked in `deployments/{network}.json`. The only committed file is `marcus.json`. `local.json` is generated by `scripts/setup-local.ts` and should not be committed (regenerated per local stack restart). Current devnet deployments:
...
Earlier devnet chains (subura, esquiline, cassius, cassius-test, monti_spl) were retired in 2026-04 → 2026-05 as the active set contracted around marcus. Their `deployments/<network>.json` artifacts and per-chain hardhat network entries were removed alongside the registry directory cleanup; CHANGELOG.md preserves the deploy history for archival reads.
...
- Test against devnet: `npx hardhat test --network marcus`.
- Oracle Gateway V2 contracts depend on live Pyth/Switchboard feeds — test against marcus for oracle-related changes.
...
| Oracle contracts | `npx hardhat test` + `npx hardhat test --network marcus` (verify live feeds) |
