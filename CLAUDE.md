...
# Refresh OG-V2 cached adapters (keeper; cron at interval < maxStaleness ~3600s)
...
| `0x46efa679` | `transfer_spl_to_signer(uint64 amount, bytes32 mint)` | Return SPL to the outer Solana tx signer's own ATA. Source = `ata(external_auth(caller), mint)`; destination = `ata(signer, mint)` where signer is the real Solana keypair (not a PDA). Signs as `external_auth(caller)`. The return leg for Solana-native users (`do_tx_unsigned` / `activate_ata` flow). Shipped in PR #223 (2026-05-26). |
...
- **`contracts/oracle/`** — Oracle Gateway V2: Chainlink-compatible adapters for Pyth Pull and Switchboard V3 price feeds. `OracleAdapterFactory` deploys `PythPullAdapter`, `SwitchboardV3Adapter`, `CachedPythAdapter`, and `CachedFeedAdapter` instances via EIP-1167 minimal proxy clones. Each adapter reads Solana account data via CPI precompile, parses Borsh-encoded price data (`PythPullParser` / `SwitchboardParser`), and normalizes to 8-decimal Chainlink format. `IExtendedOracleAdapter` extends `IAggregatorV3Interface` with confidence intervals, EMA data, and price status. `BatchReader` reads multiple feeds in one call. The factory includes owner-controlled pause/unpause emergency controls. Includes `examples/SampleLendingOracle.sol`. **Pyth Lazer surface was removed in #196 (deferral decision)** — no `lazer/` directory; no `PythLazerCache` / `PythLazerFeedAdapter` / `lazer_price` selector in this repo. **Cached feeds** (PRs #224, #225 — see `contracts/oracle/CACHED_FEEDS.md`): a keeper-driven SLOAD cache so multi-collateral Compound borrows fit Solana's 1.4M-CU per-tx cap. `CachedPythAdapter` re-parses the Pyth account directly; `CachedFeedAdapter` is a source-agnostic decorator over any `AggregatorV3` feed. `refresh()` parses + SSTOREs (keeper tx); `latestRoundData()` is a pure SLOAD (~134K CU vs ~509K parse). Refresh keeper: `scripts/oracle/refresh-cached-feeds.ts` — cron at interval < `maxStaleness` (default 3600s); reads `CACHED_FEEDS` env var or `deployments/<network>.json` `OracleGatewayV2.feeds.cachedPyth` + `.cachedFeed`. A fresh clone reverts `UninitializedPriceFeed` until first `refresh()`, then `StalePriceFeed` when aged past `maxStaleness`.
...
- `marcus` (121301) — devnet Rome chain (key: `MARCUS_PRIVATE_KEY`)
- `hadrian` (200010) — testnet Rome chain (key: `HADRIAN_PRIVATE_KEY`)
- `augustus` (200001) — testnet Rome chain (key: `AUGUSTUS_PRIVATE_KEY`)
- `aurelius` (30001) — real-testnet Rome chain (key: `AURELIUS_PRIVATE_KEY`)
- Per-chain Rome networks (`<chain>: { chainId, url, accounts: [<CHAIN>_PRIVATE_KEY] }`) are added when chains are brought up via `/bring-up-chain`. Current committed deployment artifacts: `aurelius.json`, `hadrian.json`.
...
| Cached oracle feeds (Compound CU budget) | `contracts/oracle/{CachedPythAdapter,CachedFeedAdapter}.sol` + `contracts/oracle/CACHED_FEEDS.md` + keeper `scripts/oracle/refresh-cached-feeds.ts` |
