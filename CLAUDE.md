...
**`ERC20SPLFactory` mint creation is atomic** (#326, 2026-08). `create_token_mint()` now creates AND initializes the mint in a single call (`create_and_init_mint`), binding mint authority to the caller's `EXTERNAL_AUTHORITY` PDA at creation. The former two-step flow had a permissionless `init_token_mint(mint)` window between create and init in which a third party could front-run the init on someone else's freshly-created mint and seize issuance. `init_token_mint(bytes32)` is retained as an inert `view` for backwards compat: it reads the mint's `is_initialized` byte at offset 45 and reverts if unset — it can no longer initialize or set authority on any mint. Regression: `tests/erc20spl_factory_hijack.poc.ts`. **Rubicon** carries the fixed factory at `0xe0501c6c…f984` (block 438907082); the prior factory at `0x888a485a…5409` is pre-#326 code and superseded — consumers must migrate to the new address, and the orphaned-mint sweep is tracked separately.
...
| `ERC20SPLFactory` (create/init) | `tests/erc20spl_factory.integration.ts` + `tests/erc20spl_factory_hijack.poc.ts` (front-run PoC — must revert against the fixed factory) |
