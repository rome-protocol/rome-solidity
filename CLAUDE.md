...
**`ERC20SPLFactory` atomic mint creation (#326, 2026-08).** `create_token_mint()` now creates AND initializes the SPL mint in a single call (via `HelperProgram.create_and_init_mint`) — closes a mint-authority front-running window where a third party could race `init_token_mint()` between create + init on someone else's freshly-created mint and seize issuance. `init_token_mint(bytes32)` is now an inert `view` that only asserts `is_initialized`, so existing two-step callers keep working (second call is a no-op against the already-initialized mint). The derived mint address is unchanged. Regression test: `tests/erc20spl_factory_hijack.poc.ts`. Pre-fix factories are IMMUTABLE — remediating live deployments requires a redeploy + consumer migration + orphaned-mint sweep (rubicon redeployed at `0xe0501c6c…f984` in #327; the prior `0x888a485a…5409` factory remains on-chain but superseded).
...
| `ERC20SPLFactory` (mint creation / init) | `tests/erc20spl_factory.integration.ts` + `tests/erc20spl_factory_hijack.poc.ts` (front-running regression, #326) |
