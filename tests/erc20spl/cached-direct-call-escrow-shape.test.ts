...
/// Structural regression suite for the cached wrapper — same shape as
/// direct-call-escrow-shape.test.ts (SPL_ERC20), tied to the real
/// `erc20spl_cached.sol` source and compiled artifact rather than a
/// hand-copied mirror. Behavioural coverage lives in
/// cached-transfer-behaviour.test.ts, which deploys it against the
/// precompile mocks.
describe("SPL_ERC20_cached dispatch shape — structural, source-of-truth is the file itself", function () {
    ...
    it("approve() never touches SplCached — allowance is pure EVM now", function () {
        ...
        assert.ok(!body.includes("SplCached"), "approve() must not encode a call to SplCached");
    ...
    it("_transfer's EOA-side movement is a direct CALL of transferFrom, not a delegatecall", function () {
        ...
        assert.ok(
            ...
            "no SplCached mutating selector may be reached via delegatecall from _transfer"
        );
    });
    ...
    it("isEnabled(user) reports the one-time SPL-level delegate grant, on this contract's own cached track", function () {
        assert.ok(
            ...
            "SPL_ERC20_cached must expose isEnabled(address), same as the legacy wrapper"
        );
