...
/// *deployed* on hardhat-network (see build-legacy.md); this is the
/// substitute for a behavioral test on the dispatch-mechanism change
/// itself (DELEGATECALL -> direct CALL), which a hand-copied mirror
/// cannot cover because mutating the mirror would prove nothing about the
/// production file.
///
...
describe("SPL_ERC20 dispatch shape (#511 gate) — structural, source-of-truth is the file itself", function () {
    ...
    it("_transfer routes a contract recipient's SPL to the wrapper's own ATA, never the contract's", function () {
        ...
        assert.ok(
            ...
            "this is the exact line the MUTATION step in build-legacy.md flips to prove the escrow test is load-bearing"
        );
    });
    ...
    it("bridgeOutToSolana's SPL move is a direct CALL of the delegate-ATA overload, not a delegatecall", function () {
        ...
        assert.ok(
            ...
            "Tier-1 row #4: the caller-supplied-src_ata delegate overload, direct-CALLed so the wrapper signs as the user's delegate"
        );
        assert.ok(
