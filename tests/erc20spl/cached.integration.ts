...
//     runs.
//
// The wrapper no longer exposes `mint_to` — a direct CALL would sign as
// the wrapper's own PDA, which is never the on-chain mint authority.
// Minting goes straight to the SplCached precompile (0xff..05) from the
// authority EOA; see `mintSpl` below.
//
// SCOPE:
...
describe("SPL_ERC20_cached — behavioral integration", () => {
    ...
    // The wrapper has no `mint_to` — mint straight to the SplCached
    // precompile from the authority EOA (test wallet must be the on-chain
    // mint authority for `mint_id`).
    ...
    it("approve(MaxUint256) round-trips exactly — allowance is plain uint256 EVM storage, no u64 sentinel", async () => {
        const spender = privateKeyToAccount(generatePrivateKey()).address;
...
// SplCached precompile (0xff..05) — minting bypasses the wrapper
// entirely (no ERC20SPL_cached.mint_to).
