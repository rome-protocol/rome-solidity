...
/// Structural regression suite for the Token-2022 hooked transfer: regex/
/// text assertions over the real `token2022_hooked_transfer.sol` and
/// `erc20spl_token2022_hooked.sol` sources, not a hand-copied mirror or a
/// deployed-contract test. `hooked-transfer-behaviour.test.ts` is the
/// complementary behavioral suite, deploying the real
/// `SPL_ERC20_Token2022Hooked` against `PrecompileMock`.
describe("Token2022 hooked transfer dispatch shape", function () {
    ...
    it("transferChecked invokes CpiProgram via direct CALL, not delegatecall", function () {
        ...
        assert.ok(
            ...
            "transferChecked must dispatch via a direct CALL"
        );
        assert.ok(
    ...
    it("the stale delegatecall-required rationale is gone", function () {
        assert.ok(
            ...
            "the doc comment claiming delegatecall is required must be removed — the premise is false"
        );
    });
    ...
    it("transferFromWithHookAccounts spends the caller's EVM allowance before moving tokens — the wrapper is now a fixed authority, not a per-spender SPL delegate", function () {
        ...
        assert.ok(
            ...
            "the CPI authority is always the wrapper's own PDA regardless of msg.sender, so the SPL runtime no longer gates who may call on from's behalf — the inherited EVM _allowances mapping must be the one and only access-control gate here, exactly as transferFrom already enforces for the non-hooked path"
        );
    });
